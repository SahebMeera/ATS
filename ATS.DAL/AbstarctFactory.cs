...
namespace ATS.DAL
{
    public abstract class AbstractFactory
    {
        ...
        public AbstractFactory(string ConnectionString, IConfiguration config)
        {
            ...
            using (IDbConnection db = connection)
            {
                try
                {
                    ...
                    using (var tran = db.BeginTransaction())
                    {
                        ...
                        {
                            response.Data = obj;
                            this.parms.Add("@ReturnCode", (obj as AbstractDataObject).RecordID, direction: ParameterDirection.Output, size: sizeof(int));
        ...
        public DynamicParameters SetDynamicParameters<T>(T obj)
        {
            List<string> lstIgnore = new List<string>() { "DateCreated", "DateModified" };
            Type t = obj.GetType();
            ...
            foreach (PropertyInfo property in properties)
            {
                if (!property.PropertyType.Name.Contains("List"))
                {
                    ...
                    {
                        if (attrs[0].TypeId.ToString() == "IgnoreAttribute")
                    ...
                    {
                        PropertyInfo prop = t.GetProperty(property.Name);
                        object list = prop.GetValue(obj);

                        //if (prop.PropertyType.Name == "String" && list == null)
                        //{
                        //    list = string.Empty;
                        //}

                        parameters.Add(property.Name, list);
                    }
                }
